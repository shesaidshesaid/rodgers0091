...
function* fetchTarefasSaga() {
  try {
    const response = yield call(axios.get, 'https://rodgers0091-7359ca357275.herokuapp.com/api/tarefas');
...
function* addTarefaSaga(action) {
  try {
    ...
    const response = yield call(
      axios.post,
      'https://rodgers0091-7359ca357275.herokuapp.com/api/tarefas',
      formData,
      {
        headers: {
...
function* deleteTarefaSaga(action) {
  try {
    yield call(
      axios.delete,
      `https://rodgers0091-7359ca357275.herokuapp.com/api/tarefas/${action.payload}`
    );
...
function* updateTarefaSaga(action) {
  try {
    ...
    const response = yield call(
      axios.put,
      `https://rodgers0091-7359ca357275.herokuapp.com/api/tarefas/${id}`,
      formData,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
